...
- **Gap (what's UNPROVEN):** the *real* loop has never been shown to run here. It requires (a) `opencode` CLI, (b) `mimo-v2.5` via `mimo-token-plan`, (c) a real agent-browser capability, (d) a running FE dev server + the smoke login (HMU/admin/123456). None verified present. So: state machine + envelopes + routing = PROVEN; real browser-driven test→RCA→implement→retest = UNPROVEN.
