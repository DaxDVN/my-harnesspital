...
| [kho-canonical-ui-patterns](kho-canonical-ui-patterns.md) | FE / UI (list + grid + overview) | implement,fix,review,scaffold,list-page,grid,table,filter,overview,spacing · kho,warehouse,EnhancedDataGrid,pinnedRightColumns,filterSidebar,my-filters,KpiCard,Loại A,pe-1,headerSticky | high | second-brain/kho-canonical-ui-patterns.md — single source of truth cho mọi list page & table layout sau này (ngăn lỗi Container tag, duplicate attr, gap thừa, missing pin, v.v.) |
