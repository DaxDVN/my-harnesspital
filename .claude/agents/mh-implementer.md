...
2. **Live conventions** (`harness/rules/frontend-rules-conventions-patterns.md` + `backend-rules-conventions-patterns.md`; spine: RQ-via-adapter + `useMasterData` + id-only + shadcn). Concretely: server state via module adapter (`extends GeneratedApiClient`) — **no `fetch`/`axios`/manual `JsonServiceClient`/bearer in UI**; reference data via `useMasterData([Entity])`; every list/master mutation invalidates **both** `invalidateQueries` and `invalidateMasterDataEntity(...)`; reference by `Id` not name/code; business rules from BE flags/`Constants.ts` (never hardcode strings like `=== 'kinh'`); tables → `EnhancedDataGrid`; forms → RHF+`zodResolver`+schema factory in `forms/`; status → `StatusBadge`+`STATUS_REGISTRY`; permission → `useUserPermissions().can(...)`; i18n → `useIntl`/`useTranslation`. **No FE money/stock/cost/posted/balance math.**
3. **Reuse before authoring.** Check the component-inventory / reuse-catalog and `rg` before writing a new component/hook/adapter. Mirror the warehouse exemplar's GOOD parts only (split pages, schema in `forms/`, not the monolithic/inline anti-patterns — audit §4).
