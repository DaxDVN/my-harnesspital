...
// findings .md per harness/review/findings-schema.md (workflows can't write files).
//
...
// 10 dimensions — see harness/review/checklist.md. Reviewer self-marks N/A when not applicable.
const DIMENSIONS = [
...
const reviewed = await pipeline(
  DIMENSIONS,
  (d) => agent(
    `You are the ${d.key} (${d.name}) reviewer. Read your dimension's entry in harness/review/checklist.md and review ONLY these files through that single lens:
${fileList}
Module spec dir: specs/${module}/. Enumerate before ranking. Attest coverage for EVERY file (FINDING/CLEAN/N/A). Cite live evidence for BLOCK/HIGH (rule-hit/spec/exemplar); doc-only → cap at MED. Output per harness/review/findings-schema.md.`,
