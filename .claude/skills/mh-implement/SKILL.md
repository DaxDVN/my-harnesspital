...
Bundle docs in this folder: **[preflight.md](./preflight.md)** (reuse discovery), **[definition-of-done.md](./definition-of-done.md)** (the gate). Live convention source of truth: `harness/rules/frontend-rules-conventions-patterns.md` (FE) + `backend-rules-conventions-patterns.md` (BE). FE reality spine (memory `fe-live-conventions-vs-stale-docs` + audit `velvet/notes/myhospital-fe-convention-audit-2026-06-15.md`): RQ-via-adapter + `useMasterData` + id-only + shadcn. **Do NOT trust `ARCHITECTURE-OVERVIEW.md` / `BEST-PRACTICES-NEW-PAGE.md` — both stale (audit V10).**
