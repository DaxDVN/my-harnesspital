...
OUTPUT = Path("/home/dax/Documents/arabica/roast/scripts/seed_slot2_all.sql")
CONTAINER = "mssql-hospital-wt2"
DB = "MyHospital"
SA_PASS = "Hospital123!"
TENANT = 8
HOSPITAL = 9
...
for line in cols_output.strip().splitlines():
    parts = [p.strip() for p in line.split("|")]
    if len(parts) >= 6 and parts[0] and parts[0] not in ("TABLE_NAME", "----------", ""):
        max_len = None
        if parts[3] != "NULL":
            try:
                max_len = int(parts[3])
...
def gen_value(col: dict, idx: int, table: str) -> str | None:
    ...
    if ct in ("decimal", "numeric", "float", "real"):
        ...
        if "height" in cn:
            return round(random.uniform(140, 190), 2)
