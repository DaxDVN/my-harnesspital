...
- At session start only, follow the **Session Start Protocol** in AGENTS.md: use `python scripts/worktree.py list`, then **auto-resolve the worktree from the first prompt** — if the prompt already names a worktree (slug or Vietnamese name) that maps to exactly one active worktree, proceed directly (state which one) and do NOT ask; only ask "Bạn muốn làm việc trong worktree nào?" when the prompt names none or the name is ambiguous (≥2 share it). Use fish syntax for user-facing commands. Never edit `myhospital-fe/` or `myhospital-be/` directly.
