...
- **Self-learning tiers** (full model in AGENTS.md → "Self-Learning"): `main-brain/knowledge.md` (auto-loaded above) is the lean **source of truth** — **never write `main-brain/`** (the guard hook blocks it); it changes ONLY via the owner-invoked **`/promote`** skill (explicit-only, like `/mh-review`). Capture durable cross-cutting learnings in **`second-brain/`** freely (in-repo, all tools); exact detail lives in `engine/` + `.claude/skills`.
