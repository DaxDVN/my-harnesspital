...
| **graphify** | `docs/`+`specs/` **design/business** relationships — *only when the graph is present and trusted by `harness_doctor.py`*. If absent, stale, cross-machine, or untrusted, skip graphify and read the source docs. See `engine/rules/graphify.md`. | Source code. Anything in `myhospital-fe/`, `myhospital-be/`, `worktrees/`. |
...
- **graphify** *only* for a docs/specs **design-decision / relationship** question, and
  *only* when `python scripts/harness_doctor.py` reports the graph as trusted. If absent,
  stale, cross-machine, or untrusted, skip graphify and read the source docs. Reading the
