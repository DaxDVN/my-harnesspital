...
`id` · `slice` · goal · inputs · **files-allowed-to-modify** (non-empty allowlist) · files-forbidden · steps · **validation commands** (non-empty, actually runnable) · expected artifact · dependencies · risk · **`risk_tier`** (T0|T1|T2|T3) · **`risk_class`** (clinical|billing|permission|migration|other) · rollback. No vague "implement API". The allowlist + validation are what let `/incremental-impl` run one task safely — without them the executor is unbounded.

> **`risk_tier` + `risk_class` are NORMALIZED keys, alongside (not replacing) the freeform `risk` prose.** `risk_tier` is the T0–T3 cost/criticality bucket; `risk_class` is the safety domain. Together they let the **pm-orchestrator `feature` recipe** key `scripts/tier_policy.json` deterministically PER SLICE — `tiers[risk_tier]` gives the `(model, effort)`, and `risk_class ∈ {clinical,billing,permission,migration}` applies the `risk_class_floor` and forces the per-slice owner-confirm gate. Derive them from `/impact-analysis` + the `02-requirements` per-slice risk index; default to `risk_tier: T1`, `risk_class: other` only when a slice is genuinely low-risk and reversible. A slice that touches clinical/billing/permission/migration is NEVER `other`.
