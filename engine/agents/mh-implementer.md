...
4. **Generated/contract.** Never hand-edit `generated-dtos.ts` / `Constants.ts` / `generated-api-client.ts` / EF migrations. Missing DTO/hook/constant/permission → STOP, report a contract blocker to the parent. Do not work around. **Fixing an FE bug:** ensure BE is running + parent/you ran `npm run dtos:update` (fresh DTOs) BEFORE judging the FE fix — stale-contract guard (`engine/rules/frontend.md` → "FE bug-fix prerequisite"). **Schema change:** running migrations on the slot DB is allowed if data is restorable — `make migrate-data` (`engine/rules/backend.md` → "Running migrations"); never the real DB; this is parent-coordinated, not a silent local hack.
