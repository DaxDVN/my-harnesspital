...
- **Deterministic floor:** `just mh-scan --advisory` (BE bug-classes + FE structural rules, ast-grep) — lấy làm **sàn** (mọi cái scanner bắt phải có trong findings). `just convention-truth` để biết drift.
...
- Liệt kê toàn bộ file thuộc module (MODULE BLOCK §scope) + diff committed (vs baseline) + diff uncommitted (`git -C <wt>/{be,fe} diff HEAD`).
- Chạy `just mh-scan --advisory` (lọc scope module) → ghi kết quả làm **sàn tĩnh**.
